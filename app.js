...
function updateBatchUI(){
  ...
  if(!batchMode){
    batchStatus.textContent='Mode normal : un certificat à la fois.';
    clientSignatureBlock.style.display='block';
    clientSignatureHint.textContent='';
  }else if(batchFinalized){
    batchStatus.textContent=`Tournée terminée : ${n} lit${n>1?'s':''}. Le client signe une seule fois ci-dessous, puis les certificats peuvent être générés.`;
    clientSignatureBlock.style.display='block';
    clientSignatureHint.textContent='✍️ Cette signature sera automatiquement apposée sur tous les certificats de la tournée.';
  }else{
    batchStatus.textContent=n===0?'Tournée démarrée : réalise le premier lit puis enregistre-le.':`${n} lit${n>1?'s':''} enregistré${n>1?'s':''}. Réalise le lit suivant puis enregistre-le.`;
