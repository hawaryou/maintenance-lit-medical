...
/* ==========================================================
   MODE MAINTENANCE DE PLUSIEURS LITS — signature client unique
   ========================================================== */
let batchMode=false;
...
const multiModeCard=document.getElementById('multiModeCard');
const batchSave=document.getElementById('batchSave');
...
const clientSignatureBlock=document.getElementById('clientSignatureBlock');
const clientSignatureHint=document.getElementById('clientSignatureHint');

function updateBatchUI(){
  const n=batchRecords.length;
  const pdfButton=document.getElementById('pdf');
  const sendButton=document.getElementById('send');
  batchCount.textContent=`${n} lit${n>1?'s':''}`;
  document.body.classList.toggle('batch-active',batchMode);
  normalModeCard?.classList.toggle('active',!batchMode);
  multiModeCard?.classList.toggle('active',batchMode);
  batchStart.disabled=batchMode;
  batchSave.disabled=!batchMode||batchFinalized;
  batchFinish.disabled=!batchMode||n===0||batchFinalized;
  batchCancel.disabled=!batchMode;
  if(pdfButton) pdfButton.disabled=batchMode&&!batchFinalized;
  if(sendButton) sendButton.disabled=batchMode&&!batchFinalized;
  if(pdfButton) pdfButton.textContent=batchFinalized?'📄 Générer le dossier PDF':'Générer le PDF';
  if(sendButton) sendButton.textContent=batchFinalized?'📧 Dossier PDF + E-mail':'Générer le PDF et envoyer par e-mail';
  if(!batchMode){
    batchStatus.innerHTML='🟢 <strong>MODE NORMAL ACTIF</strong> — 1 lit à la fois, avec signature client pour ce certificat.';
    clientSignatureBlock.style.display='block';
    clientSignatureHint.textContent='';
  }else if(batchFinalized){
    batchStatus.innerHTML=`🟣 <strong>MODE MULTI-LITS — SIGNATURE UNIQUE</strong> · ${n} lit${n>1?'s':''} enregistrés. Le client signe une seule fois ci-dessous, puis les certificats peuvent être générés.`;
    clientSignatureBlock.style.display='block';
    clientSignatureHint.textContent='✍️ Cette signature sera automatiquement apposée sur tous les certificats de la tournée.';
  }else{
    batchStatus.innerHTML=n===0?'🟣 <strong>MODE MULTI-LITS ACTIF</strong> — Réalisez le premier lit puis enregistrez-le.':`🟣 <strong>MODE MULTI-LITS ACTIF</strong> — ${n} lit${n>1?'s':''} enregistré${n>1?'s':''}. Réalisez le lit suivant puis enregistrez-le.`;
    clientSignatureBlock.style.display='none';
    clientSignatureHint.textContent='';
  }
}
function clearCanvas(canvas){const ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height)}
function resetBedFieldsForNext(){
  ['serie','serieBar','seriePot','annee','observations','pieces'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});
  document.querySelectorAll('input[name="barrieres"],input[name="potence"],input[name="env"]').forEach(el=>el.checked=false);
  sections.forEach(([title,items],si)=>items.forEach((it,i)=>{document.querySelectorAll(`input[name="c_${si}_${i}"]`).forEach(el=>el.checked=false)}));
  const obs=document.getElementById('obsolescence'); if(obs){obs.value='1';obs.dispatchEvent(new Event('input'))}
  clearCanvas(sigClient);
  window.scrollTo({top:0,behavior:'smooth'});
}
function startBatch(){
  batchMode=true;batchFinalized=false;batchRecords=[];clearCanvas(sigClient);updateBatchUI();showMessage('Tournée multi-lits démarrée.','success');window.scrollTo({top:0,behavior:'smooth'});
}
function saveCurrentBedToBatch(){
  if(!batchMode||batchFinalized)return;
  const d=collect();
  if(!d.serie.trim()){showMessage('Renseignez le N° de série / parc du lit avant de l’enregistrer.','error');document.getElementById('serie')?.focus();return}
  d.sigClient='';
  batchRecords.push(d);
  updateBatchUI();
  showMessage(`Lit ${batchRecords.length} enregistré. Préparez le lit suivant.`,'success');
  resetBedFieldsForNext();
}
function finishBatch(){
  if(!batchMode)return;
  const currentSerie=val('serie').trim();
  if(currentSerie&&!batchFinalized){ saveCurrentBedToBatch(); }
  if(batchRecords.length===0){showMessage('Enregistrez au moins un lit avant de terminer la tournée.','error');return;}
  batchFinalized=true;updateBatchUI();
  document.getElementById('clientSignatureBlock')?.scrollIntoView({behavior:'smooth',block:'center'});
  showMessage('La tournée est terminée. Faites signer le client une seule fois.','success');
}
function cancelBatch(){
  if(!batchMode)return;
  const ok=window.confirm(`Annuler la tournée et supprimer les ${batchRecords.length} lit${batchRecords.length>1?'s':''} enregistrés ?`);
  if(!ok)return;
  batchMode=false;batchFinalized=false;batchRecords=[];clearCanvas(sigClient);updateBatchUI();showMessage('Tournée multi-lits annulée.','info');
}
...
function applyClientSignatureToBatch(){
  if(!canvasHasInk(sigClient)) throw new Error('La signature du client est nécessaire avant de générer le dossier.');
  const sig=sigClient.toDataURL('image/png');
  return batchRecords.map(d=>({...d,sigClient:sig}));
}

function selectNormalMode(){
  if(batchMode){
    const ok=window.confirm('Revenir au mode normal ? Les lits déjà enregistrés dans la tournée seront supprimés.');
    if(!ok)return;
    batchMode=false;batchFinalized=false;batchRecords=[];clearCanvas(sigClient);
  }
  updateBatchUI();
  showMessage('Mode normal sélectionné : 1 lit à la fois.','info');
}

batchStart?.addEventListener('click',startBatch);
normalMode?.addEventListener('click',selectNormalMode);
batchSave?.addEventListener('click',saveCurrentBedToBatch);
batchFinish?.addEventListener('click',finishBatch);
...
function renderPDFPage(pdf,d){
  const W=210,M=10;
  pdf.setFont('helvetica');pdf.setFontSize(8);pdf.text('ENR08-V05',W-M,8,{align:'right'});
  pdf.addImage(document.querySelector('.top img').src,'JPEG',M,5,48,17);
  pdf.setFontSize(14);pdf.setFont('helvetica','bold');pdf.text('FICHE DE MAINTENANCE LIT MÉDICALISÉ',W/2,29,{align:'center'});
  pdf.setFontSize(8);pdf.setFont('helvetica','normal');pdf.text(`Maintenance : ${d.maintenance||'—'}`,W/2,35,{align:'center'});
  let y=41;
  pdf.setDrawColor(100);pdf.rect(M,y,W-2*M,40);
  pdf.setFont('helvetica','bold');pdf.text('IDENTIFICATION',M+3,y+5);pdf.text('LOCALISATION',110,y+5);
  pdf.setFont('helvetica','normal');
  const left=[`Marque / modèle : ${d.marque}`,`Année de fabrication : ${d.annee}`,`N° série / parc : ${d.serie}`,`Barrières : ${d.barrieres}   N° : ${d.serieBar}`,`Potence : ${d.potence}   N° : ${d.seriePot}`,`Environnement : ${d.env}`];
  const right=[`Établissement / client : ${d.client}`,`Service / chambre : ${d.localisation}`,`Adresse : ${d.adresse}`,`Ville : ${d.ville}`,`Réf. intervention : ${d.reference}`];
  ...
  if(hasNonConforme){
    pdf.setFillColor(255,205,205);pdf.roundedRect(M,y,W-2*M,8,1.5,1.5,'F');pdf.setTextColor(190,0,0);pdf.setFont('helvetica','bold');pdf.setFontSize(10);pdf.text('MATÉRIEL NON CONFORME',W/2,y+5.4,{align:'center'});pdf.setTextColor(0,0,0);pdf.setFontSize(8);y+=11;
  }else{
    pdf.setFillColor(210,245,220);pdf.roundedRect(M,y,W-2*M,8,1.5,1.5,'F');pdf.setTextColor(20,125,55);pdf.setFont('helvetica','bold');pdf.setFontSize(10);pdf.text('MATÉRIEL CONFORME',W/2,y+5.4,{align:'center'});pdf.setTextColor(0,0,0);pdf.setFontSize(8);y+=11;
  }

  for(const [title,items] of sections){
    if(y>265){pdf.addPage();y=12}
    pdf.setFont('helvetica','bold');pdf.setFillColor(232,238,241);pdf.rect(M,y,W-2*M,6,'F');pdf.text(title,M+2,y+4.2);y+=7;pdf.setFont('helvetica','normal');
    for(const it of items){
      if(y>270){pdf.addPage();y=12}
      pdf.rect(M,y,W-2*M,7);pdf.text(it,M+2,y+4.5,{maxWidth:128});
      const result=(d.controls||{})[it]||'—';
      if(result==='Non conforme'){
        pdf.setFillColor(255,185,185);pdf.roundedRect(166,y+1,32,5,1,1,'F');pdf.setTextColor(180,0,0);pdf.setFont('helvetica','bold');pdf.text(result,182,y+4.5,{align:'center'});pdf.setTextColor(0,0,0);pdf.setFont('helvetica','normal');
      }else if(result==='Conforme'){
        pdf.setFillColor(210,245,220);pdf.roundedRect(166,y+1,32,5,1,1,'F');pdf.setTextColor(20,125,55);pdf.setFont('helvetica','bold');pdf.text(result,182,y+4.5,{align:'center'});pdf.setTextColor(0,0,0);pdf.setFont('helvetica','normal');
      }else{pdf.text(result,172,y+4.5,{align:'center'})}
      y+=7;
    }
    y+=2;
  }

  if(y>240){pdf.addPage();y=12}
  pdf.setFont('helvetica','bold');pdf.text('COMMENTAIRES / OBSERVATIONS :',M,y+4);pdf.text('PIÈCES CHANGÉES :',110,y+4);pdf.rect(M,y+6,90,35);pdf.rect(110,y+6,90,35);
  const obsColor={1:[46,125,50],2:[139,195,74],3:[251,192,45],4:[239,108,0],5:[198,40,40]}[Number(d.obsolescence)]||[120,120,120];
  pdf.setFillColor(...obsColor);pdf.roundedRect(M+2,y+8,42,8,1.5,1.5,'F');pdf.setTextColor(255,255,255);pdf.setFont('helvetica','bold');pdf.text(`OBSOLESCENCE : ${d.obsolescence||'—'} / 5`,M+23,y+13.3,{align:'center'});pdf.setTextColor(0,0,0);pdf.setFont('helvetica','normal');
  const obsText=pdf.splitTextToSize(d.observations||'',84);pdf.text(obsText,M+3,y+21);pdf.text(pdf.splitTextToSize(d.pieces||'',86),112,y+11);y+=45;
  pdf.text(`Rappel des consignes de sécurité : ${d.securite||'—'}`,M,y);pdf.text('* suivant norme NF EN 60601-2-52',M,y+5);pdf.text(`Fait le : ${d.date||'—'}    Technicien : ${d.technicien||'—'}`,M,y+12);
  pdf.setFont('helvetica','bold');pdf.text('Signature du technicien',M,y+19);pdf.text('Signature du client',110,y+19);pdf.rect(M,y+21,90,30);pdf.rect(110,y+21,90,30);
  if(d.sigTech)pdf.addImage(d.sigTech,'PNG',M+2,y+23,86,26);if(d.sigClient)pdf.addImage(d.sigClient,'PNG',112,y+23,86,26);
}

function makePDFForData(d,pdf){
  if(!pdf)pdf=new window.jspdf.jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
  renderPDFPage(pdf,d);
  return {pdf,data:d};
}
async function makePDF(){return makePDFForData(collect())}
async function makeBatchPDF(){
  if(!batchFinalized||batchRecords.length===0)throw new Error('Aucune tournée finalisée.');
  const records=applyClientSignatureToBatch();
  const {jsPDF}=window.jspdf;
  const pdf=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
  records.forEach((d,i)=>{if(i>0)pdf.addPage();renderPDFPage(pdf,d)});
  return {pdf,data:records,records};
}

async function generateCurrentOrBatchPDF(){
  if(batchFinalized)return makeBatchPDF();
  return makePDF();
}

document.getElementById('pdf').onclick=async()=>{
  try{
    const result=await generateCurrentOrBatchPDF();
    if(batchFinalized){
      result.pdf.save(`ENR08-V05_Tournee_${new Date().toISOString().slice(0,10)}_${result.records.length}-lits.pdf`);
      showMessage(`${result.records.length} certificats regroupés dans un seul PDF. Signature client unique appliquée.`,'success');
    }else{
      const data=result.data;result.pdf.save(`ENR08-V05_${data.serie||'maintenance'}_${data.date||''}.pdf`);
    }
  }catch(e){console.error(e);showMessage(e.message||'Erreur lors de la génération du PDF.','error')}
};
...
document.getElementById('send').onclick=async()=>{
  ...
  try{
    ...
    const result=await generateCurrentOrBatchPDF();
    const data=batchFinalized?result.records[0]:result.data;
    const filename=batchFinalized?`ENR08-V05_Tournee_${new Date().toISOString().slice(0,10)}_${result.records.length}-lits.pdf`:`ENR08-V05_${data.serie||'maintenance'}_${data.date||''}.pdf`;
    if(!APPS_SCRIPT_URL || !APPS_SCRIPT_URL.startsWith('https://script.google.com/macros/s/')){
      ...
      result.pdf.save(filename);return;
    }
    status.textContent=batchFinalized?'Envoi du dossier de maintenance…':'Envoi du PDF…';
    const b64=result.pdf.output('datauristring').split(',')[1];
    const params=new URLSearchParams({token:APP_TOKEN,pdf_base64:b64,filename,client:data.client,technicien:data.technicien,date:data.date,reference:data.reference,serie:batchFinalized?`${result.records.length} lits`:data.serie});
    await fetch(APPS_SCRIPT_URL,{method:'POST',mode:'no-cors',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:params.toString()});
    status.textContent=batchFinalized?`Dossier de ${result.records.length} lits transmis à ${EMAIL_TO}. Signature client unique appliquée.`:`Demande d’envoi transmise à ${EMAIL_TO}. Vérifiez la boîte de réception.`;
  }catch(e){
    console.error(e);status.textContent='Erreur lors de la transmission. Le PDF va être téléchargé.';
    try{const result=await generateCurrentOrBatchPDF();const filename=batchFinalized?`ENR08-V05_Tournee_${new Date().toISOString().slice(0,10)}_${result.records.length}-lits.pdf`:`ENR08-V05_${result.data.serie||'maintenance'}_${result.data.date||''}.pdf`;result.pdf.save(filename)}catch(_){}
  }
};
